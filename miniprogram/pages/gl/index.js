...
Page({
  onLoad() {
    ...
    q.select('#gl').fields({ node: true, size: true }).exec(res => {
      ...
      // ---- 可调参数 ----
      const EXPOSURE   = 1.35         // 画面亮度
      const LIGHT_MAIN = 1.15         // 主光强度
      const LIGHT_AMBI = 0.30         // 环境光强度
      const OFFSET_Y   = -0.55        // 地球下移（屏幕下三分之一）
      const RADIUS     = 1            // 地球半径
      const MARGIN     = 1.02         // 初始让球刚好卡在两侧，再略放大一点点

      const ROT_SPEED    = 0.005      // 拖拽旋转速度
      const PITCH_CLAMP  = Math.PI/2 - 0.01
      const Z_MIN        = 1.4        // 最近/最远缩放
      const Z_MAX        = 6.0
      const WHEEL_ZOOM_STEP = 0.15    // 鼠标滚轮缩放步长

      const TAP_MOVE_PX = 6           // 点击判定阈值
      ...
      function lonlatToVec3(lon, lat, r = RADIUS + 0.001) {
        ...
        return new THREE.Vector3(-x, y, z)      // 与贴图方向对齐
      }
      ...
      // 顶在最前：线材质 / 面材质（关闭 depthTest，使用 renderOrder）
      const makeLineMat = (color, ro=30) => {
        const m = new THREE.LineBasicMaterial({ color, transparent:true, opacity:0.96, depthTest:false })
        m.depthWrite = false; m.userData = { ro }; return m
      }
      ...
      loader.load('../../assets/textures/earth.jpg', (tex)=>{
        ...
        let BORDER_GROUP   = null     // 常显白色边界
        let HIGHLIGHT_GROUP = null    // 点击高亮（黄边+半透明填充）

        // 只从 countries.json 读取（你已经把 geojson 转成 json 放好了）
        const loadCountries = () => new Promise((resolve, reject) => {
          if (COUNTRY_FEATURES) return resolve(COUNTRY_FEATURES)
          try {
            const gj = require('../../assets/data/countries.json')   // <<== 关键：用 .json
        ...
        function drawBorders() {
          ...
          const SKIP_EVERY = 1  // 如需降采样减负，可设为 2/3/4…
        ...
        this.onTouchMove = e => {
          ...
          const dxFactor = isPC ? -1 : 1   // 左右在 PC 反向
          const dyFactor = isPC ? +1 : -1  // 上下在 PC 反向
